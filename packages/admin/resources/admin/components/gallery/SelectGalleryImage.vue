...
const activeTabProps = computed(() => {
  if (activeTab.value === 'upload') {
    return {
      ...
      submitting: uploadSubmitting.value,
    }
  }
  return {
    images: images.value,
    ...
    submitting: selectSubmitting.value,
  }
})
...
async function handleConfirmSelect(payload) {
  if (selectSubmitting.value) return
  try {
...
async function handleConfirmUpload(payload) {
  if (uploadSubmitting.value) return
  try {
    uploadSubmitting.value = true
    if (props.onSelect) {
      await props.onSelect({
        file: payload?.file ?? uploadFile.value,
