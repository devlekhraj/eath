		...
		:loading="fetching_data"
			...
			<template #item.name="{ item }">
				<div style="min-width: max-content;">
					<router-link :to="{ name: 'adminBannerDetailPage', params: { id: item.id } }" class="text-primary text-capitalize text-decoration-underline">
						{{ item.name }}
					</router-link>
				</div>
			</template>
			...
			<template #item.image_count="{ item }">
				<div style="min-width: max-content;">
					<v-chip :color="item.image_count ? 'primary' : 'grey'" size="small" label class="text-capitalize">
			...
			<template #item.actions="{ item }">
				<div class="d-flex align-center ga-2" style="min-width: max-content;">
					<v-btn size="x-small" icon variant="tonal" color="primary" :to="{ name: 'adminBannerDetailPage', params: { id: item.id } }">
						<v-icon size="16">mdi-eye</v-icon>
					</v-btn>
					<v-btn size="x-small" icon variant="tonal" color="warning" @click="handleOpen(item)">
