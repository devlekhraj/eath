<template>
    <v-card>
        <v-card-title class="d-flex align-center justify-space-between py-0">
            <span>{{ props.item.id ? 'Update Banner':'Add New Banner' }}</span>
        ...
        <v-card-text>
            <v-form ref="formRef" @submit.prevent="handleSubmit" lazy-validation>
                <v-row>
                    <v-col cols="12" md="12">
                        <v-text-field v-model="form.name" label="Banner Name" :rules="[rules.required]" :error-messages="serverErrors.name" required />
                    </v-col>
                    ...
                    <v-col cols="6" md="6">
                        <v-text-field v-model="form.aspect_ratio" label="Image Aspect Ratio" type="number" placeholder="2.4 aspect ratio for 1920x800" :error-messages="serverErrors.aspect_ratio" />
                    </v-col>
...
onMounted(() => {
    ...
    if (props.item?.id) {
        Object.assign(form, {
            ...
            aspect_ratio: props.item.aspect_ratio || null,
...
async function handleSubmit() {
    try {
        ...
        loading.value = true
        const resp = await http.post('admin/banners', form)
