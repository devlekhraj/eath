            ...
            <v-data-table :items="filteredItems" :headers="headers" :loading="loading" class="elevation-0"
                ...
                <template #item.name="{ item }">
                    <div class="d-flex align-center ga-2" style="min-width: max-content;">
                        ...
                        <span class="text-capitalize">{{ item.fname }} {{ item.lname }}</span>
                    </div>
                </template>
                                ...
                                <v-row>
                                    <v-col cols="12" md="3">
                                        ...
                                        <div>
                                            ...
                                            <div class="mb-2">
                                                <span>Name:</span> {{ item.fname }} {{ item.lname }}
                                            </div>
                                            <div class="mb-2">
                                                <span>Mobile:</span> {{ item.mobile_no || 'N/A' }}
                                            </div>
                                            <div class="mb-2">
                                                <span>Email:</span> {{ item.email || 'N/A' }}
                                            </div>
                                        </div>
                                    </v-col>
                                    <v-col cols="12" md="9">
                ...
                <template #item.actions="{ item }"> <div> <v-btn icon size="x-small" color="primary" variant="tonal" class="ml-2" @click="toggleExpand(item)"> <v-icon> {{ expanded.includes(item.id) ? 'mdi-chevron-up' : 'mdi-chevron-down' }} </v-icon> </v-btn> </div> </template>
            </v-data-table>
        </div>
