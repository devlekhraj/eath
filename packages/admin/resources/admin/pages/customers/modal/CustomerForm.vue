<template>
  <v-card>
    <v-card-title class="d-flex align-center justify-space-between py-0">
        ...
        <span>{{ item && item.id ? 'Update Customer Profile' : 'Add New Customer' }}</span>
      </div>
    ...
    <v-card-text class="pt-8">
      <v-form ref="formRef" @submit.prevent="handleSubmit" lazy-validation>
        <v-row>
          <!-- Name -->
          <v-col cols="12" md="6">
            <v-text-field v-model="form.fname" prepend-inner-icon="mdi-account" label="First Name" :rules="[rules.required]" :error-messages="serverErrors.fname" required />
          </v-col>
          <v-col cols="12" md="6">
            <v-text-field v-model="form.lname" prepend-inner-icon="mdi-account" label="First Name" :rules="[rules.required]" :error-messages="serverErrors.lname" required />
          </v-col>
          <v-col cols="12" md="12">
            <v-text-field v-model="form.username" prepend-inner-icon="mdi-account" label="Username" :rules="[rules.required, rules.username]" :error-messages="serverErrors.username" required />
          </v-col>
          ...
          <!-- Phone -->
          <v-col cols="12" md="6">
            <v-text-field v-model="form.mobile_no" prepend-inner-icon="mdi-phone" label="Phone" :rules="[rules.required, rules.phone]" :error-messages="serverErrors.mobile_no" required />
          </v-col>
