    ...
    <v-card>
        <v-card-title class="d-flex align-center justify-space-between py-0">
            <div class="d-flex align-center justify-space-between">
                <div>
                    <span>{{item.id ? "Update":"Add New"}} Question</span>
                </div>
                <div>
                </div>
            </div>
        ...
        <v-card-text>
            <v-form ref="formRef" @submit.prevent="submitForm" lazy-validation>
                <v-row>
                    <v-col cols="12">
                        <v-text-field v-model="form.question" label="Question" :rules="[rules.required]" />
                    </v-col>

                    <v-col cols="12">
                        <v-textarea v-model="form.answer" label="Answer" :rules="[rules.required]" />
                    </v-col>
                    <v-col cols="6">
                        <v-text-field type="number" label="Sequence No#" v-model="form.sort_order"></v-text-field>
                    </v-col>
                    <v-col cols="6">
