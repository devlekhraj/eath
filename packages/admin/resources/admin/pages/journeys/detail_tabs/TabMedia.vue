      ...
      <v-card>
        ...
        <v-card-text class="pt-4 text-center">
          ...
          <div class="text-caption text-grey mt-2">
            Are you sure you want to remove this image from the journey? This will unlink it from this collection.
          </div>
        </v-card-text>
...
function openEditModal(targetType, item) {
  ...
  editDialog.targetLabel = targetType === 'hero'
    ? 'Hero Banner'
    : targetType === 'card'
    ? 'Card Thumbnail'
...
async function handleSaveEdit() {
  if (!props.journey?.id || !editDialog.item) return
...
function promptRemoveImage(targetType, item) {
  ...
  deleteDialog.targetLabel = targetType === 'hero'
    ? 'Hero Banner'
    : targetType === 'card'
    ? 'Card Thumbnail'
    : 'Route Map'
...
async function executeRemove() {
  ...
  try {
    if (deleteDialog.targetType === 'gallery') {
      await detachJourneyMediaApi(props.journey.id, deleteDialog.item.attachment_id)
      showSuccess('Photo removed from gallery.')
    } else {
      const isHero = deleteDialog.targetType === 'hero'
      const isCard = deleteDialog.targetType === 'card'
      const fieldKey = isHero ? 'hero_image_id' : isCard ? 'card_image_id' : 'route_map_image_id'
      const label = isHero ? 'Hero banner' : isCard ? 'Card thumbnail' : 'Route map'
