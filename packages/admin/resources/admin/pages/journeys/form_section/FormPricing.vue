                    ...
                    <div>
                        ...
                        <div>
                            <v-row>
                                <v-col cols="7">
                                    <div>
                                        <v-text-field label="Price Name" v-model="form.name" prepend-inner-icon="mdi-invoice-list" :rules="[rules.required]"></v-text-field>
                                    </div>
                        ...
                        <v-table density="compact" class="mt-2">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-left text-primary">Primary</th>
                            ...
                            <tbody>
                                <tr v-for="(item, index) in travelPackage.prices" :key="index">
                                    ...
                                    <td class="py-2">
                                        <v-chip size="small" :color="item.is_primary || item.is_default ? 'primary':'error'">{{ item.is_primary || item.is_default ? 'Yes':'No' }}</v-chip>
                                    </td>
                                    <td class="py-2">
...
async function handleSubmit() {
    ...
    try {
        ...
        showSuccess(resp.message || 'Price saved successfully')
        emit('refresh')
    } catch (err) {
