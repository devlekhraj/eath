        ...
        <v-card-text>
            <v-form ref="formRef" @submit.prevent="handleSubmit" lazy-validation>
                <v-row>
                    <!-- Name Field -->
                    <v-col cols="12">
                        <v-select v-model="form.code" :items="lookup_codes" label="Code"></v-select>
                    </v-col>
                    <v-col cols="12">
                        <v-text-field v-model="form.name" label="Item Name" :rules="[rules.required]" :error-messages="serverErrors.name" required />
                    </v-col>
