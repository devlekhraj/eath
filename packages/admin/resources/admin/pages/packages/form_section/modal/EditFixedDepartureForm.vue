		...
		<v-card-text class="px-14 pt-10">
			<v-form ref="formRef" @submit.prevent="submit">
				<v-row dense class="align-center">
					<v-col cols="12" md="6">
						<v-date-input v-model="formData.start_date" class="w-100" label="Start Date" prepend-icon="" :min="minStartIso" :rules="[rules.required]" :display-value="formatHuman(formData.start_date)" :menu-props="{ maxWidth: 360, minWidth: 0 }" />
					</v-col>
					<v-col cols="12" md="6">
						<v-date-input v-model="formData.end_date" class="w-100" label="End Date" prepend-icon="" :min="formData.start_date || minStartIso" :rules="[rules.required, (v) => rules.endAfterStart(v, formData.start_date)]"
							:display-value="formatHuman(formData.end_date)"
							:menu-props="{ maxWidth: 360, minWidth: 0 }"
						/>
					</v-col>
					<v-col cols="12" md="6">
						<v-text-field v-model="formData.total_seat" type="number" min="0" label="Total Seat" :rules="[rules.required]" />
					</v-col>
					<v-col cols="12" md="6">
						<v-text-field v-model="formData.cost" type="number" min="0" label="Cost" prefix="$" :rules="[rules.required]" />
					</v-col>
					<v-col cols="12" md="6">
