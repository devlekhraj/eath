...
{
    public function saveImage(Request $request, $destinationId)
    {
        ...
        $result['gallery']->usages()->updateOrCreate(
            [
                'usage_type' => $destination->getTable(),
                'usage_id' => $destination->id,
            ],
            [
    ...
    public function useImage(Request $request, $destinationId)
    {
        $destination = Destination::find($destinationId);
        if (! $destination) {
            return response()->json([
