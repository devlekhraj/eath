...
@section('title', 'When to Trek in Nepal · Month-by-Month Guide · EATH Website')
...
<div class="website-container" style="padding-top: var(--space-6); padding-bottom: var(--space-12);">
    ...
    <header style="margin-bottom: var(--space-8); max-width: 860px;">
        <div style="display: flex; align-items: center; gap: var(--space-2); margin-bottom: var(--space-2); flex-wrap: wrap;">
            ...
            <span class="website-badge website-badge--neutral">12 Months</span>
        </div>

        <h1 class="website-h1" style="margin: 0 0 var(--space-3) 0;">
            When to Trek in Nepal: Month-by-Month Guide
        </h1>

        <p class="website-body website-text-secondary" style="margin: 0 0 var(--space-4) 0; font-size: 1.125rem; line-height: 1.6;">
            Himalayan trekking conditions shift markedly across elevation zones and seasons. Use this interactive calendar to review seasonal patterns, historical trail rhythms, and catalog itineraries matched to each travel month.
        </p>

        <!-- Explicit Sample Seasonality Disclaimer -->
        <div class="website-notice website-notice--info" style="margin: 0; padding: var(--space-3) var(--space-4);">
            <span class="website-micro" style="display: block; line-height: 1.5;">
                <strong>Sample Seasonality Notice:</strong> Monthly suitability profiles reflect historical climatic patterns and catalog fixture tags. They do not constitute real-time weather forecasts, guarantee trail passability, or confirm departure operation. Independent route verification is essential before real-world travel.
            </span>
        </div>
    </header>
                ...
                <a href="{{ route('website.months.index', ['month' => $m['slug']]) }}"
                   ...
                   aria-current="{{ $isSelected ? 'true' : 'false' }}">
                    <div class="website-month-tile__header">
                        ...
                        @if($isSelected)
                            <span class="website-badge website-badge--primary" style="font-size: 0.7rem; padding: 2px 6px;">
                    ...
                    <div style="margin-bottom: var(--space-2);">
                        <span class="website-micro website-text-muted" style="text-transform: uppercase; font-weight: 600; letter-spacing: 0.05em;">
                            {{ $m['season'] }}
                        </span>
                    </div>
    ...
    <section aria-labelledby="heading-seasons" style="margin-bottom: var(--space-10);">
        <div style="margin-bottom: var(--space-4);">
            <span class="website-badge website-badge--accent" style="margin-bottom: var(--space-1);">Climatic Framework</span>
            ...
            @foreach($seasons as $season)
                <div class="website-card" style="padding: var(--space-5); background: var(--color-surface); border: 1px solid var(--color-border);">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--space-2);">
                        <strong class="website-body" style="color: var(--color-primary-dark);">{{ $season['name'] }}</strong>
                        <span class="website-badge website-badge--neutral" style="font-size: 0.72rem;">{{ $season['months'] }}</span>
                    </div>
        ...
        <div class="website-card" style="padding: var(--space-6); background: var(--color-background-warm); border: 1px solid var(--color-border);">
            <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: var(--space-3); margin-bottom: var(--space-4);">
                <div>
                    <div style="display: flex; align-items: center; gap: var(--space-2); margin-bottom: var(--space-1);">
                        <span class="website-badge website-badge--primary">{{ $selectedMonth['season'] }} Window</span>
                        @if($isDefault && $selectedMonth['id'] === 9)
                            <span class="website-badge website-badge--neutral">Sample Default</span>
                        @endif
                    </div>
                ...
                <!-- Link to dedicated Phase 19 Month Detail Page -->
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: var(--space-3); margin-bottom: var(--space-2);">
                <div style="padding: var(--space-3) var(--space-4); background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-md);">
                    <strong class="website-micro" style="text-transform: uppercase; color: var(--color-text-muted); display: block; margin-bottom: var(--space-1);">
    ...
    <section aria-labelledby="heading-matching-treks" style="margin-bottom: var(--space-12);">
        <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: var(--space-5); flex-wrap: wrap; gap: var(--space-3);">
            ...
            @if(count($matchingTreks) > 0)
                <a href="{{ route('website.treks.index', ['month' => $selectedMonth['id']]) }}" class="website-btn website-btn--outline" style="font-size: 0.875rem;">
            ...
            <div class="website-card" style="padding: var(--space-8); text-align: center; background: var(--color-surface); border: 1px dashed var(--color-border);">
                <div style="max-width: 540px; margin: 0 auto;">
                    ...
                    <div style="display: flex; justify-content: center; gap: var(--space-3); flex-wrap: wrap;">
                        ...
                        <a href="{{ route('website.months.index', ['month' => 'october']) }}" class="website-btn website-btn--outline">
    ...
    <section aria-labelledby="heading-plan-month-cta" style="margin-bottom: var(--space-10);">
        <div class="website-card" style="padding: var(--space-8); background: var(--color-primary-subtle); border: 2px solid var(--color-primary-light); text-align: center;">
            ...
            <h2 id="heading-plan-month-cta" class="website-h2" style="margin-bottom: var(--space-2); color: var(--color-primary-dark);">
                Plan a {{ $selectedMonth['name'] }} Himalayan Trek
            </h2>
            <p class="website-body website-text-secondary" style="max-width: 680px; margin: 0 auto var(--space-6) auto; line-height: 1.6;">
                Launch our interactive trek planner with {{ $selectedMonth['name'] }} pre-selected as your target window. Customize your preferred duration, party size, and difficulty comfort to explore matching itineraries.
            </p>

            <div style="display: flex; justify-content: center; gap: var(--space-4); flex-wrap: wrap;">
                <a href="{{ route('website.planner.start') }}?mode=discover&month={{ $selectedMonth['id'] }}&source=month" class="website-btn website-btn--primary">
                    Plan {{ $selectedMonth['name'] }} Trek &rarr;
                </a>
    ...
    @if($article)
        <section aria-labelledby="heading-guide-article">
            <div class="website-card" style="padding: var(--space-6); background: var(--color-surface); border: 1px solid var(--color-border);">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: var(--space-3);">
                    <div style="max-width: 720px;">
                        <span class="website-badge website-badge--neutral" style="margin-bottom: var(--space-2);">Planning Guide</span>
                    ...
                    <a href="{{ route('website.articles.show', ['slug' => $article['slug']]) }}" class="website-btn website-btn--outline" style="font-size: 0.875rem;">
