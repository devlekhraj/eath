<template>
	<v-row class="mt-2">
		<v-col cols="12" md="6">
			<v-text-field v-model="searchQuery" label="Search image..." variant="outlined" density="comfortable"
				prepend-inner-icon="mdi-magnify" clearable class="mb-3" />

			<div class="select-grid-scroll">
				<v-row>
					<v-col v-for="(image, index) in filteredImages" :key="image?.id || index" cols="6" sm="4" md="4">
				...
				<div v-if="!filteredImages.length" class="text-center py-6">
					<p>No images found.</p>
				</div>
			</div>
		</v-col>
		...
		<v-col cols="12" md="6">
			...
			<div class="mb-3">
				<v-text-field :model-value="meta?.alt_text" label="Alt Text" variant="outlined" density="comfortable"
					:disabled="!selectedImage" :error-messages="fieldErrors.alt_text || []"
					@update:modelValue="(value) => updateMetaField('alt_text', value)" />
			</div>
			<div class="mb-3">
				<v-text-field :model-value="meta?.caption" label="Caption" variant="outlined" density="comfortable"
					:disabled="!selectedImage" :error-messages="fieldErrors.caption || []"
					@update:modelValue="(value) => updateMetaField('caption', value)" />
			</div>
			<div class="mb-3">
				<v-textarea :model-value="meta?.description" label="Description" variant="outlined" density="comfortable"
					rows="4" auto-grow :disabled="!selectedImage" :error-messages="fieldErrors.description || []"
	...
	<v-card-actions class="justify-space-around">
		<v-btn color="primary" :disabled="!selectedImage || submitting" :loading="submitting" @click="confirmSelect">
			Select
		</v-btn>
	</v-card-actions>
</template>
...
const filteredImages = computed(() => {
	...
	return (props.images || []).filter((image) => {
		const name = (image?.name || image?.filename || image?.title || '').toLowerCase()
