<template>
    <v-card flat>
        <v-card-title class="d-flex align-center">
            <span class="font-medium">{{ props.item.id ? 'Update Banner':'Add New Banner' }}</span>
            <v-spacer></v-spacer>
            <v-btn icon variant="text" color="error" @click="handleCancel" size="small"><v-icon>mdi-close</v-icon></v-btn>
        </v-card-title>
                ...
                <v-row>
                    ...
                    <v-col cols="6" md="6">
                        <v-text-field
                            ...
                            type="number"
                            placeholder="2.4 aspect ratio for 1920x800"
                            variant="outlined"
                            density="comfortable"
                            :error-messages="serverErrors.aspect_ratio"
                        />
                    </v-col>
...
async function handleSubmit() {
    try {
        ...
        loading.value = true
        const resp = await axios.post('admin/banners', form)
