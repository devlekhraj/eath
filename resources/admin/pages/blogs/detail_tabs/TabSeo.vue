        ...
        <v-row>
            <v-col cols="12" lg="8" offset-lg="2">
                <v-text-field v-model="form.meta_title" label="Meta Title" variant="outlined" density="comfortable"
                    counter="60" hint="Max 60 characters for best SEO" persistent-hint class="mb-4"
                    :error-messages="errors.meta_title" />

                <v-textarea v-model="form.meta_description" label="Meta Description" variant="outlined"
                    density="comfortable" rows="3" counter="160" hint="Max 160 characters for search engines"
                    persistent-hint class="mb-4" :error-messages="errors.meta_description" />

                <v-textarea v-model="seoKeyword" label="Meta Keyword" variant="outlined" density="comfortable"
                    rows="2" hint="Separate keywords with commas" persistent-hint class="mb-4"
                    :error-messages="errors.meta_keyword || errors.meta_keywords" />

                    <div class="text-center mt-6">
                        <v-btn type="button" color="primary" size="large" :loading="isSubmitting" @click="submitSeo">
                            Submit
                        </v-btn>
                    </div>
            </v-col>
