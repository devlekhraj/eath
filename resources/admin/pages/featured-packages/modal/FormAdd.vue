    ...
    <v-card flat>
        <v-card-title>
            <div class="d-flex align-center">
                <span class="font-medium">{{ item?.id ? 'Edit' : 'Create' }} Featured Package</span>
                <v-spacer></v-spacer>
                <v-btn variant="text" icon size="small" @click="handleCancel"
                    color="error"><v-icon>mdi-close</v-icon></v-btn>
            </div>
        </v-card-title>
                ...
                <v-row>
                    ...
                    <v-col cols="12">
                        <v-text-field v-model="form.title" label="Title" density="comfortable" variant="outlined"
                            hide-details :rules="[rules.required]" :error-messages="serverErrors.title"
                    ...
                    <!-- Slug -->
                    <v-col cols="12">
                        <v-select v-model="form.package_id" :items="package_list" item-title="name" item-value="id"
                            label="Select Package" density="comfortable" variant="outlined" :rules="[rules.required]"
                    ...
                    <!-- Start Date -->
                    <v-col cols="12" md="6">
                        <v-date-input prepend-icon="" v-model="form.start_date" label="Start Date" density="comfortable"
                            variant="outlined" :rules="[rules.required, rules.startDate]"
                            :error-messages="serverErrors.start_date" prepend-inner-icon="mdi-calendar-star"
                            :min="today" :max="form.end_date || undefined" />
                    </v-col>

                    <!-- End Date -->
                    <v-col cols="12" md="6">
                        <v-date-input prepend-icon="" v-model="form.end_date" label="End Date" density="comfortable"
                            variant="outlined" :rules="[rules.required, rules.endDate(form.start_date)]"
                            :error-messages="serverErrors.end_date" prepend-inner-icon="mdi-calendar"
                            :min="form.start_date || undefined" />
                    </v-col>

                    <!-- Group Size -->
                    <v-col cols="12" md="6">
                        <v-text-field v-model="form.group_size" label="Group Size" type="number" density="comfortable"
                            variant="outlined" :rules="[rules.required, rules.positiveNumber]" hide-details
                            :error-messages="serverErrors.group_size" prepend-inner-icon="mdi-account-group" />
                    </v-col>

                    <!-- Price -->
                    <v-col cols="12" md="6">
                        <v-text-field v-model="form.price" label="Price" type="number" density="comfortable"
                            variant="outlined" :rules="[rules.required, rules.positiveNumber]" hide-details
                            :error-messages="serverErrors.price" prepend-inner-icon="mdi-currency-usd" />
                    </v-col>
                    <!-- Price -->
                    <v-col cols="12" md="12">
                    ...
                    <v-col cols="12" md="12">
                        <div>
                            <v-file-input v-if="item?.id" v-model="selected_image" accept="image/*" prepend-icon=""
                                label="Featured Image" variant="outlined" density="comfortable"
