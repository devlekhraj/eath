    ...
    <v-card flat>
        <v-card-title>
            <div class="w-100 d-flex justify-between align-center">
                <span class="font-medium">Itinerary Lookup Form</span>
                <v-spacer></v-spacer>
                 <v-btn size="small" icon variant="text" color="error" @click="handleCancel"><v-icon>mdi-close</v-icon></v-btn>
            </div>
        </v-card-title>
                ...
                <v-row>
                    ...
                    <v-col cols="12">
                        <v-file-input label="Upload Icon Image" accept="image/*" variant="outlined" prepend-icon=""
                            density="comfortable" :error-messages="serverErrors.icon_url"
                            @change="handleIconUpload" prepend-inner-icon="mdi-upload" required />
...
async function handleSubmit() {
    try {
        loading.value = true
        const resp = await axios.post('admin/itinerary-lookups', form)
