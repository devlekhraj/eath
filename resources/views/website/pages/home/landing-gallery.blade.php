@php
    $images = $images ?? ($galleryImage->image_urls ?? []);
    $title = $title ?? 'Himalayan Moments';
    $subtitle = $subtitle ?? 'Summit mornings, alpine trails, and mountain villages.';
    $badge = $badge ?? 'Gallery';
@endphp
    ...
    <section class="py-12 sm:pt-16 bg-white pb-0">
        <div class="max-w-7xl mx-auto px-2 sm:px-3 lg:px-4">
            <div class="flex items-end justify-between gap-6">
                <div>
                    <span class="inline-flex items-center text-sm font-semibold text-sky-600">{{ $badge }}</span>
                    <h2 class="mt-3 text-3xl sm:text-4xl font-semibold text-slate-900">{{ $title }}</h2>
        ...
        <div class="mt-8 px-2 sm:px-3 lg:px-4">
            <div class="relative">
                <div class="swiper gallery-swiper px-2 sm:px-4">
                ...
                <button type="button"
                    class="hidden sm:flex absolute -left-3 top-1/2 z-10 -translate-y-1/2 items-center justify-center rounded-full bg-sky-600 text-white p-3 shadow-lg ring-2 ring-white/70 transition hover:bg-sky-700 hover:shadow-xl"
                    aria-label="Previous" data-gallery-swiper-prev>
                ...
                <button type="button"
                    class="hidden sm:flex absolute -right-3 top-1/2 z-10 -translate-y-1/2 items-center justify-center rounded-full bg-sky-600 text-white p-3 shadow-lg ring-2 ring-white/70 transition hover:bg-sky-700 hover:shadow-xl"
                    aria-label="Next" data-gallery-swiper-next>
                    <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"
                        stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
