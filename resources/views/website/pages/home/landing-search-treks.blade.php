                    ...
                    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-[1.2fr_1fr_1fr_auto]">
                        ...
                        <select id="trek-region" name="region"
                            ...
                            @foreach(collect($destinations ?? [])->sortBy('name') as $destination)
                                <option value="{{ $destination['slug'] }}" {{ request('region') == $destination['slug'] ? 'selected' : '' }}>
                                    {{ $destination['name'] }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select id="trek-duration" name="duration"
                            class="w-full rounded-md border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-200">
                            <option value="" {{ request('duration') == '' ? 'selected' : '' }}>Any duration</option>
                            <option value="short" {{ request('duration') == 'short' ? 'selected' : '' }}>6–10 days</option>
                            <option value="medium" {{ request('duration') == 'medium' ? 'selected' : '' }}>11–14 days</option>
                            <option value="long" {{ request('duration') == 'long' ? 'selected' : '' }}>15+ days</option>
                        </select>
                        <button type="submit" id="trek-search-btn"
                            ...
                            <svg id="trek-search-icon" class="mr-2 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-4.35-4.35m0 0A7.5 7.5 0 104.5 4.5a7.5 7.5 0 0012.15 12.15z" />
                            </svg>
                            <svg id="trek-search-spinner" class="mr-2 h-5 w-5 animate-spin text-white hidden" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <div class="border-slate-200 py-6">
                ...
                <div class="mt-4" id="trek-results">
                    ...
                    <div class="divide-y divide-slate-200">
                        @php
                            $rows = $searchResults ?? $hotPackages;
                        @endphp
                        @foreach ($rows as $item)
                            <div class="grid gap-3 py-4 sm:grid-cols-[1.7fr_0.8fr_0.8fr_0.6fr_0.8fr] sm:items-center">
                                ...
                                <div class="flex sm:justify-end">
                                    <a href="{{ route('trek.show', ['destination' => $item->destination->slug, 'slug' => $item->slug]) }}"
